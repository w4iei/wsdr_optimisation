...
def _mss_wsdr(s_true, e_spat, e_interf, e_artif, error_scaling_variables):
    """Measurement of the separation quality for a given source in terms of
    filtered true source, interference and artifacts. Using weights. "Weighted SDR"
    """
    e_spat_scale, e_interf_scale, e_artif_scale = error_scaling_variables
    assert len(error_scaling_variables) == 3
    # assert sum(error_scaling_variables) == 3
    # energy ratios
    energy_s_true = np.sum((s_true)**2)
    sdr = _safe_db(energy_s_true,
                   np.sum((e_spat + e_interf + e_artif)**2))
    wsdr = _safe_db(energy_s_true,
                    np.sum((e_spat_scale*e_spat + e_interf_scale*e_interf + e_artif_scale*e_artif)**2))
    return sdr, wsdr, 0, 0  # 0 is better than np.nan because all np.nan slices make things angry
